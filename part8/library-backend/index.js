...
const resolvers = {
  Query: {
    ...
    allBooks: (root, args) => {
      if (args.author !== "") {
        const temp = books.filter((book) => book.author === args.author);
        if (args.genre !== "") {
          return temp.filter((book) => book.genres.includes(args.genre));
        } else {
          return temp;
        }
      } else if (args.genre !== "") {
  ...
  Mutation: {
    ...
    editAuthor: (root, args) => {
      if (authors.filter((author) => author.name === args.name) === 0) {
        return null;
      } else {
